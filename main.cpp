...
int main(int argc, char const *argv[]){
    ...
    net.addLayer(new Conv2DLayer(std::pair(28,28),std::pair(4,4),4,0,std::pair(4,4),learningRate,batchSize));
    net.addLayer(new LinearLayer(196,100,learningRate,batchSize));
    ...
    Matrix error = Matrix(10,1);
    Matrix pred = Matrix(10,1);
    ...
    for(int reps = 0; reps < NUM_REPS; reps++){
        ...
        for(MNISTLabel label: data){
            ...
            pred = net.feedFoward(label._in);

            if(pred.maxIndex() == actual.maxIndex())
                batchCorrect++;

            error = crossEntropyLossDeriv(pred,actual);
            ...
            if(batch % batchSize == 0){
                ...
                if(printCounter % PRINT_FREQ == 0) {
                    ...
                    std::cout << "REP : " << reps << ", loss : " << sum(lossTotal) / (batchSize * 10 * PRINT_FREQ)
                              ...
                              << "  , time taken : " << timer.getCurrentTimeInSeconds() << "\n";
                    totalTime += timer.getCurrentTimeInSeconds();
                    timer.start();
                    batchCorrect = 0;
                    clear(lossTotal);
                }
